...
module.exports = class DigiComm{
	...
	dataIn(d){
		if(!this.temp.length && d != 126){
			if(this.at_wait){
				this.temp.push(d);
				var data = this.temp;
				var cmd = String.fromCharCode(data[1])+String.fromCharCode(data[2]);
				if(this.parent.at_wait == cmd){
					this.parent.at_wait = false;
					var frame = {
						command: cmd,
						data: frame.length > 2 ? frame.slice(2) : false
					}
					this._emitter.emit('digi-response:'+, frame);
					this.temp = [];
					this.at_wait = false;
				}
			}else{
	...
	send_at_raw(cmd, param, expected){
		var frame = [];
		for(var i=0;i<command.length;i++) frame.push(command.charCodeAt(i));
		if(!expected) expected = 1;
		this.at_wait = cmd;
		if(typeof param == 'undefined') return this._send(frame, expected);
		if(param.constructor != Array) param = [param];
		frame.push(...param);
		return this._send(frame, expected);
	}
...
class incomingFrame{
	...
	transmit_status(frame){
		return {
			...
			hasError: (frame[4] > 0 || frame[2] == 253)
		}
	}
