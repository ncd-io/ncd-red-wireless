...
class incomingFrame{
	...
	route_information_packet(frame){
		return {
			...
			ack_timouts: frame[6],
			tx_blocked: frame[7],
			destination_mac: frame.slice(9, 17).toMac(),
			source_mac: frame.slice(17, 25).toMac(),
			responder_mac: frame.slice(25, 33).toMac(),
			receiver_mac: frame.slice(33, 41).toMac(),
		}
	}
	aggregate_addressing_update(frame){
		return {
			format: frame[0],
			new_address: frame.slice(1, 9).toMac(),
			old_address: frame.slice(9, 17).toMac(),
		}
	}
	receive_packet(frame){
		return {
			mac: frame.slice(0, 8).toMac(),
	...
	explicit_rx_indicator(frame){
		return {
			source_mac: frame.slice(0, 8).toMac(),
	...
	io_data_sample_indicator(frame){
		return {
			...
			analog_samples: frame.slice(17).chunk(2).map((i) => msbLsb(i[0], i[1]))
		}
	}
	node_identification_indicator(frame){
		var packet = {
			source_mac: frame.slice(0, 8).toMac(),
			receive_options: this.receiveOptions(frame[10]),
			remote_mac: frame.slice(13, 21).toMac(),
	...
	remote_command_response(frame){
		return {
			id: frame[0],
			remote_mac: frame.slice(1, 9).toMac(),
...
Array.prototype.toMac = function(){
	return this.reduce((h,c,i) => {return (i==1?h.toHex():h)+':'+c.toHex();});
}
Array.prototype.chunk = function(l){
	var arr = [];
	for(i=0;i<this.length;i+=l){
