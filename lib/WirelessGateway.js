...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			console.log('emiting to control_send, emit topic('+frame.type+'-'+frame.mac+')');
			// this._emitter.emit(type+'-'+frame.mac, {error: 'Bad Payload', error_message: 'Bad payload type:'+type});
			this._emitter.emit(frame.type+'-'+frame.mac, data);
		}
	}
	power_up(payload){
		return {
	...
	sensor_data(payload){
		var parsed = {
			...
			battery_percent: msbLsb(payload[2], payload[3]) * 0.537 - 449.9,
			counter: payload[4],
		...
		return new Promise((fulfill, reject) => {
			that.queue.add(() => {
				return new Promise((f, r) => {
					...
					that._emitter.once('receive_packet-'+sensor_mac, pass);
					console.log('Listening for: receive_packet-'+sensor_mac);
