		...
		'80': {
			...
			parse: (d) => {
				...
				return {
					...
					x_rms: signInt(d.slice(4, 6).reduce(msbLsb), 16),
					x_max: signInt(d.slice(6, 8).reduce(msbLsb), 16),
					x_velocity: signInt(d.slice(8, 10).reduce(msbLsb), 16) / 100,
					x_displacement: signInt(d.slice(10, 12).reduce(msbLsb), 16) / 100,
					x_peak_one: signInt(d.slice(12, 14).reduce(msbLsb), 16),
					x_peak_two: signInt(d.slice(14, 16).reduce(msbLsb), 16),
					x_peak_three: signInt(d.slice(16, 18).reduce(msbLsb), 16),

					y_rms: signInt(d.slice(18, 20).reduce(msbLsb), 16),
					y_max: signInt(d.slice(20, 22).reduce(msbLsb), 16),
					y_velocity: signInt(d.slice(22, 24).reduce(msbLsb), 16) / 100,
					y_displacement: signInt(d.slice(24, 26).reduce(msbLsb), 16) / 100,
					y_peak_one: signInt(d.slice(26, 28).reduce(msbLsb), 16),
					y_peak_two: signInt(d.slice(28, 30).reduce(msbLsb), 16),
					y_peak_three: signInt(d.slice(30, 32).reduce(msbLsb), 16),

					z_rms: signInt(d.slice(32, 34).reduce(msbLsb), 16),
					z_max: signInt(d.slice(34, 36).reduce(msbLsb), 16),
					z_velocity: signInt(d.slice(36, 38).reduce(msbLsb), 16) / 100,
					z_displacement: signInt(d.slice(38, 40).reduce(msbLsb), 16) / 100,
					z_peak_one: signInt(d.slice(40, 42).reduce(msbLsb), 16),
					z_peak_two: signInt(d.slice(42, 44).reduce(msbLsb), 16),
					z_peak_three: signInt(d.slice(44, 46).reduce(msbLsb), 16),
				};
			}
		},
		'81': {
