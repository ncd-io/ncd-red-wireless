...
module.exports = class WirelessSensor{
	...
	sensor_data(payload, frame){
		...
		if(payload.length == 179){
			...
			if(typeof this.sensor_types[parsed.sensor_type] == 'undefined'){
				...
				parsed.sensor_data = this.sensor_types[parsed.sensor_type].parse(payload, parsed, frame.mac, this);
	...
	build_80_data(payload, deviceAddr, hour, minute, sdata_start, current_packet, firmware){
		if(current_packet != 1){
			console.log('bad packet cleanup');
			return;
		}
			...
			parse: (payload, parsed, mac, this) => {
				...
				if(payload[8] == 1){
					...
					if(globalDevices.hasOwnProperty(deviceAddr)){
						...
						if(current_packet in globalDevices[deviceAddr].data || current_packet == 1 || !(((current_packet&127)-1) in globalDevices[deviceAddr].data)) {
							...
							if(current_packet != 1){
								...
								this.build_80_data(payload, deviceAddr, hour, minute, sdata_start, current_packet, firmware);
					...
					return {
						...
						x_rms_ACC_mg: payload.slice(12, 14).reduce(msbLsb)/1000,
						x_max_ACC_mg: payload.slice(14, 16).reduce(msbLsb)/1000,
						...
						y_rms_ACC_mg: payload.slice(26, 28).reduce(msbLsb)/1000,
						y_max_ACC_mg: payload.slice(28, 30).reduce(msbLsb)/1000,
						...
						z_rms_ACC_mg: payload.slice(40, 42).reduce(msbLsb)/1000,
						z_max_ACC_mg: payload.slice(42, 44).reduce(msbLsb)/1000,
