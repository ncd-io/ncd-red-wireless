...
function sensor_types(parent){
	var types = {
		...
		'61':{
			...
			parse: (d) => {
				return {
					ph: signInt(d.slice(0, 1).reduce(msbLsb), 24)/1000,
					temperature: d.slice(2, 3).reduce(msbLsb)/100
				}
			}
		},
		'62':{
			name: 'ORP Temperature Sensor',
			parse: (d) => {
				return {
					ORP: signInt(d.slice(0, 1).reduce(msbLsb), 24)/1000,
					temperature: d.slice(2, 3).reduce(msbLsb)/100
				}
			}
		},
		'63':{
			name: 'pH ORP Temperature Sensor',
			parse: (d) => {
				return {
					ORP: signInt(d.slice(0, 1).reduce(msbLsb), 24)/1000,
					ph: signInt(d.slice(2, 3).reduce(msbLsb), 24)/1000,
					temperature: d.slice(4, 5).reduce(msbLsb)/100
				}
			}
		},
		'64': {
...
=======
>>>>>>> 28f2a6ca515c2bb66a4bfb65a5083ab2c4aea974
		'510': {
			name: 'GreenLight',
			parse: (d) => {
				var adc = d.slice(0, 2).reduce(msbLsb);
