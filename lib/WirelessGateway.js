...
module.exports = class WirelessSensor{
	...
	sensor_data(payload, frame){
		...
		if(payload.length == 179){
			...
			}else if(this.sensor_types[parsed.sensor_type] == 80){
				parsed = this.sensor_types[parsed.sensor_type].parse(payload, parsed);
				if(!parsed){
					return;
				}
			...
			parse: (payload, parsed) => {
				...
				if(payload[8] == 1){
					...
					return {
						...
						x_velocity_mm_sec: d.slice(8, 10).reduce(msbLsb) / 100,
						...
						y_velocity_mm_sec: d.slice(22, 24).reduce(msbLsb) / 100,
						...
						z_velocity_mm_sec: d.slice(36, 38).reduce(msbLsb) / 100,
