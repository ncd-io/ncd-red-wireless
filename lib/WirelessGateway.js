...
module.exports = class WirelessSensor{
	...
	sensor_data(payload, frame){
		...
		if(payload.length == 179){
			...
			}else if(parsed.sensor_type == 80 || parsed.sensor_type == 82 ){
		...
		'81': {
			...
			parse: (d) => {
				...
				return {
					...
					x2_rms_ACC_G: d.slice(49, 51).reduce(msbLsb)/1000,
					x2_max_ACC_G: d.slice(51, 53).reduce(msbLsb)/1000,
					x2_velocity_mm_sec: d.slice(53, 55).reduce(msbLsb) / 100,
					x2_displacement_mm: d.slice(55, 57).reduce(msbLsb) / 100,
					x2_peak_one_Hz: d.slice(57, 59).reduce(msbLsb),
					x2_peak_two_Hz: d.slice(59, 61).reduce(msbLsb),
					x2_peak_three_Hz: d.slice(61, 63).reduce(msbLsb),

					y2_rms_ACC_G: d.slice(63, 65).reduce(msbLsb)/1000,
					y2_max_ACC_G: d.slice(65, 67).reduce(msbLsb)/1000,
					y2_velocity_mm_sec: d.slice(67, 69).reduce(msbLsb) / 100,
					y2_displacement_mm: d.slice(69, 71).reduce(msbLsb) / 100,
					y2_peak_one_Hz: d.slice(71, 73).reduce(msbLsb),
					y2_peak_two_Hz: d.slice(73, 75).reduce(msbLsb),
					y2_peak_three_Hz: d.slice(75, 77).reduce(msbLsb),

					z2_rms_ACC_G: d.slice(77, 79).reduce(msbLsb)/1000,
					z2_max_ACC_G: d.slice(79, 81).reduce(msbLsb)/1000,
					z2_velocity_mm_sec: d.slice(81, 83).reduce(msbLsb) / 100,
					z2_displacement_mm: d.slice(83, 85).reduce(msbLsb) / 100,
					z2_peak_one_Hz: d.slice(85, 87).reduce(msbLsb),
					z2_peak_two_Hz: d.slice(87, 89).reduce(msbLsb),
					z2_peak_three_Hz: d.slice(89, 91).reduce(msbLsb),
				};
			}
		},
