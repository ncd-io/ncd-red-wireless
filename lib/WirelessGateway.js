...
function sensor_types(parent){
	var types = {
		...
		'17': {
			...
			parse: (d) => {
				return {
					voltage: d.slice(0, 3).reduce(msbLsb) / 32
				};
			}
		},
		'18': {
		...
		'19': {
			...
			parse: (d) => {
				return {
					...
					channel_2: d.slice(3, 6).reduce(msbLsb),
				};
			}
		},
		'20': {
			name: 'Precision Pressure & Temperature',
			parse: (d) => {
				return {
		...
		'21': {
			...
			parse: (d) => {
				return {
					psi: signInt(d.slice(0, 2).reduce(msbLsb), 16) / 100,
		...
		'28': {
			...
			parse: (d) => {
				return {
					channel_1: d.slice(0, 3).reduce(msbLsb),
					channel_2: d.slice(3, 6).reduce(msbLsb),
					channel_3: d.slice(6, 9).reduce(msbLsb)
				};
			}
		},
		'29': {
