		...
		'81': {
			...
			parse: (d) => {
				return {
					...
					x2_rms: signInt(d.slice(46, 47).reduce(msbLsb), 16),
					x2_max: signInt(d.slice(48, 49).reduce(msbLsb), 16),
					x2_velocity: signInt(d.slice(50, 51).reduce(msbLsb), 16) * 100,
					x2_displacement: signInt(d.slice(52, 53).reduce(msbLsb), 16) * 100,
					x2_peak_one: signInt(d.slice(54, 55).reduce(msbLsb), 16),
					x2_peak_two: signInt(d.slice(56, 57).reduce(msbLsb), 16),
					x2_peak_three: signInt(d.slice(58, 59).reduce(msbLsb), 16),

					y2_rms: signInt(d.slice(60, 61).reduce(msbLsb), 16),
					y2_max: signInt(d.slice(62, 63).reduce(msbLsb), 16),
					y2_velocity: signInt(d.slice(64, 65).reduce(msbLsb), 16) * 100,
					y2_displacement: signInt(d.slice(66, 67).reduce(msbLsb), 16) * 100,
					y2_peak_one: signInt(d.slice(68, 69).reduce(msbLsb), 16),
					y2_peak_two: signInt(d.slice(70, 71).reduce(msbLsb), 16),
					y2_peak_three: signInt(d.slice(72, 73).reduce(msbLsb), 16),

					z2_rms: signInt(d.slice(74, 75).reduce(msbLsb), 16),
					z2_max: signInt(d.slice(76, 77).reduce(msbLsb), 16),
					z2_velocity: signInt(d.slice(78, 79).reduce(msbLsb), 16) * 100,
					z2_displacement: signInt(d.slice(80, 81).reduce(msbLsb), 16) * 100,
					z2_peak_one: signInt(d.slice(82, 83).reduce(msbLsb), 16),
					z2_peak_two: signInt(d.slice(84, 85).reduce(msbLsb), 16),
					z2_peak_three: signInt(d.slice(86, 87).reduce(msbLsb), 16),
				};
			}
		},
		'101':{
