		...
		'80': {
			...
			parse: (d) => {
				...
				return {
					...
					x_rms: d.slice(4, 6).reduce(msbLsb),
					x_max: d.slice(6, 8).reduce(msbLsb),
					x_velocity: d.slice(8, 10).reduce(msbLsb) / 100,
					x_displacement: d.slice(10, 12).reduce(msbLsb) / 100,
					x_peak_one: d.slice(12, 14).reduce(msbLsb),
					x_peak_two: d.slice(14, 16).reduce(msbLsb),
					x_peak_three: d.slice(16, 18).reduce(msbLsb),

					y_rms: d.slice(18, 20).reduce(msbLsb),
					y_max: d.slice(20, 22).reduce(msbLsb),
					y_velocity: d.slice(22, 24).reduce(msbLsb) / 100,
					y_displacement: d.slice(24, 26).reduce(msbLsb) / 100,
					y_peak_one: d.slice(26, 28).reduce(msbLsb),
					y_peak_two: d.slice(28, 30).reduce(msbLsb),
					y_peak_three: d.slice(30, 32).reduce(msbLsb),

					z_rms: d.slice(32, 34).reduce(msbLsb),
					z_max: d.slice(34, 36).reduce(msbLsb),
					z_velocity: d.slice(36, 38).reduce(msbLsb) / 100,
					z_displacement: d.slice(38, 40).reduce(msbLsb) / 100,
					z_peak_one: d.slice(40, 42).reduce(msbLsb),
					z_peak_two: d.slice(42, 44).reduce(msbLsb),
					z_peak_three: d.slice(44, 46).reduce(msbLsb),
				};
			}
		},
		'81': {
