					...
					if(globalDevices.hasOwnProperty(deviceAddr)){
						...
						if(Object.keys(globalDevices[deviceAddr].data).length == expected_packets){
							...
							for(var i = 0; i < raw_data.length; i+=en_axis_data.increment){
								...
								if('x_offset' in en_axis_data){
									fft_concat[label].x = parseFloat((signInt(((raw_data[i+en_axis_data.x_offset]<<8)+(raw_data[i+en_axis_data.x_offset+1])), 16)*globalDevices[deviceAddr].fsr).toFixed(5));
								}
								if('y_offset' in en_axis_data){
									fft_concat[label].y = parseFloat((signInt(((raw_data[i+en_axis_data.y_offset]<<8)+(raw_data[i+en_axis_data.y_offset+1])), 16)*globalDevices[deviceAddr].fsr).toFixed(5));
								}
								if('z_offset' in en_axis_data){
									fft_concat[label].z = parseFloat((signInt(((raw_data[i+en_axis_data.z_offset]<<8)+(raw_data[i+en_axis_data.z_offset+1])), 16)*globalDevices[deviceAddr].fsr).toFixed(5));
								}
							...
							var fft_concat_obj = {
								...
								mac_address: deviceAddr,
								en_axis: globalDevices[deviceAddr].en_axis,
								en_axis: globalDevices[deviceAddr].fsr,
								odr: globalDevices[deviceAddr].odr,
								device_temp: globalDevices[deviceAddr].device_temp,
								data: fft_concat
							};
