		...
		'80': {
			...
			parse: (d) => {
				...
				return {
					...
					x_rms: d.slice(4, 6).reduce(msbLsb)/1000,
					x_max: d.slice(6, 8).reduce(msbLsb)/1000,
					...
					y_rms: d.slice(18, 20).reduce(msbLsb)/1000,
					y_max: d.slice(20, 22).reduce(msbLsb)/1000,
					...
					z_rms: d.slice(32, 34).reduce(msbLsb)/1000,
					z_max: d.slice(34, 36).reduce(msbLsb)/1000,
		...
		'81': {
			...
			parse: (d) => {
				...
				return {
					...
					x1_rms: d.slice(4, 6).reduce(msbLsb),
					x1_max: d.slice(6, 8).reduce(msbLsb),
					x1_velocity: d.slice(8, 10).reduce(msbLsb) / 100,
					x1_displacement: d.slice(10, 12).reduce(msbLsb) / 100,
					x1_peak_one: d.slice(12, 14).reduce(msbLsb),
					x1_peak_two: d.slice(14, 16).reduce(msbLsb),
					x1_peak_three: d.slice(16, 18).reduce(msbLsb),

					y1_rms: d.slice(18, 20).reduce(msbLsb),
					y1_max: d.slice(20, 22).reduce(msbLsb),
					y1_velocity: d.slice(22, 24).reduce(msbLsb) / 100,
					y1_displacement: d.slice(24, 26).reduce(msbLsb) / 100,
					y1_peak_one: d.slice(26, 28).reduce(msbLsb),
					y1_peak_two: d.slice(28, 30).reduce(msbLsb),
					y1_peak_three: d.slice(30, 32).reduce(msbLsb),

					z1_rms: d.slice(32, 34).reduce(msbLsb),
					z1_max: d.slice(34, 36).reduce(msbLsb),
					z1_velocity: d.slice(36, 38).reduce(msbLsb) / 100,
					z1_displacement: d.slice(38, 40).reduce(msbLsb) / 100,
					z1_peak_one: d.slice(40, 42).reduce(msbLsb),
					z1_peak_two: d.slice(42, 44).reduce(msbLsb),
					z1_peak_three: d.slice(44, 46).reduce(msbLsb),

					s2_odr: odr2,
					s2_temperature: signInt(d.slice(47, 49).reduce(msbLsb), 16) / 100,

					x2_rms: d.slice(49, 51).reduce(msbLsb),
					x2_max: d.slice(51, 53).reduce(msbLsb),
					x2_velocity: d.slice(53, 55).reduce(msbLsb) / 100,
					x2_displacement: d.slice(55, 57).reduce(msbLsb) / 100,
					x2_peak_one: d.slice(57, 59).reduce(msbLsb),
					x2_peak_two: d.slice(59, 61).reduce(msbLsb),
					x2_peak_three: d.slice(61, 63).reduce(msbLsb),

					y2_rms: d.slice(63, 65).reduce(msbLsb),
					y2_max: d.slice(65, 67).reduce(msbLsb),
					y2_velocity: d.slice(67, 69).reduce(msbLsb) / 100,
					y2_displacement: d.slice(69, 71).reduce(msbLsb) / 100,
					y2_peak_one: d.slice(71, 73).reduce(msbLsb),
					y2_peak_two: d.slice(73, 75).reduce(msbLsb),
					y2_peak_three: d.slice(75, 77).reduce(msbLsb),

					z2_rms: d.slice(77, 79).reduce(msbLsb),
					z2_max: d.slice(79, 81).reduce(msbLsb),
					z2_velocity: d.slice(81, 83).reduce(msbLsb) / 100,
					z2_displacement: d.slice(83, 85).reduce(msbLsb) / 100,
					z2_peak_one: d.slice(85, 87).reduce(msbLsb),
					z2_peak_two: d.slice(87, 89).reduce(msbLsb),
					z2_peak_three: d.slice(89, 91).reduce(msbLsb),
				};
			}
		},
		'101':{
