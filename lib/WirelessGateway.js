	...
	sensor_data(payload, frame){
		...
		if(parsed.sensor_type == 101){
			...
			if(globalDevices.hasOwnProperty(deviceAddr)){
				...
				var device_temp = msbLsb(payload[13], payload[14]);
				...
				if(firmware > 0){
					var probe_temp = msbLsb(payload[15], payload[16]);
			...
			if(globalDevices.hasOwnProperty(deviceAddr)){
				...
				if(Object.keys(globalDevices[deviceAddr].data).length == expected_packets){
					...
					for(var i = 0; i < raw_data.length; i+=2){
						label++;
						fft_concat[label] = {'v': parseFloat((signInt(((raw_data[i]<<8)+(raw_data[i+1])), 16)*.00006).toFixed(5))};
					}
					var fft_concat_obj = {
						time_id: globalDevices[deviceAddr].hour +':'+ globalDevices[deviceAddr].minute,
						mac_address: deviceAddr,
				...
				// var odr;
				var odr = payload[9];
				var device_temp = msbLsb(payload[12], payload[13]);
				var probe_temp = msbLsb(payload[14], payload[15]);
