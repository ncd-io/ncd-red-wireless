...
console.log(odr);
	...
	// build_515_data(bank, bank_total, mac){
	// 	console.log('Fire 3');
	// 	let sensor_data = {};
		...
		'515': {
			...
			parse: (d, parsed, mac) => {
				...
				if(!globalDevices.hasOwnProperty(mac)) {
					...
					// let bad_stream = false;
					// If we can only recover the last item in a bank
					 // just consider the stream lost
					 // to recover the last bank we would need to async call
					 // this method again and it is not worth the complexity
					 // unless this is absolutely necessary
					// Object.keys(globalDevices[mac].data.forEach(key => {
					// 	if(bank < key){
					// 		console.log('Back Packet Detect. Sending Partial.');
					// 		console.log(bank);
					// 		console.log(key);
					// 		bad_stream = true;
					// 		break;
