...
module.exports = class WirelessSensor{
	...
	sensor_data(payload, frame){
		var parsed = {
			...
			battery_percent: (msbLsb(payload[2], payload[3]) * 0.537 - 449.9).toFixed(3),
			counter: payload[4],
		...
		'80': {
			...
			parse: (d) => {
				...
				return {
					...
					odr: odr,
					temperature: signInt(d.slice(2, 3).reduce(msbLsb), 16) / 100,

					x_rms: signInt(d.slice(4, 5).reduce(msbLsb), 16),
					x_max: signInt(d.slice(6, 7).reduce(msbLsb), 16),
					x_velocity: signInt(d.slice(8, 9).reduce(msbLsb), 16) / 100,
					x_displacement: signInt(d.slice(10, 11).reduce(msbLsb), 16) / 100,
					x_peak_one: signInt(d.slice(12, 13).reduce(msbLsb), 16),
					x_peak_two: signInt(d.slice(14, 15).reduce(msbLsb), 16),
					x_peak_three: signInt(d.slice(16, 17).reduce(msbLsb), 16),

					y_rms: signInt(d.slice(18, 19).reduce(msbLsb), 16),
					y_max: signInt(d.slice(20, 21).reduce(msbLsb), 16),
					y_velocity: signInt(d.slice(22, 23).reduce(msbLsb), 16) / 100,
					y_displacement: signInt(d.slice(24, 25).reduce(msbLsb), 16) / 100,
					y_peak_one: signInt(d.slice(26, 27).reduce(msbLsb), 16),
					y_peak_two: signInt(d.slice(28, 29).reduce(msbLsb), 16),
					y_peak_three: signInt(d.slice(30, 31).reduce(msbLsb), 16),

					z_rms: signInt(d.slice(32, 33).reduce(msbLsb), 16),
					z_max: signInt(d.slice(34, 35).reduce(msbLsb), 16),
					z_velocity: signInt(d.slice(36, 37).reduce(msbLsb), 16) / 100,
					z_displacement: signInt(d.slice(38, 39).reduce(msbLsb), 16) / 100,
					z_peak_one: signInt(d.slice(40, 41).reduce(msbLsb), 16),
					z_peak_two: signInt(d.slice(42, 43).reduce(msbLsb), 16),
					z_peak_three: signInt(d.slice(44, 45).reduce(msbLsb), 16),
				};
			}
		},
		'81': {
			...
			parse: (d) => {
				...
				return {
					...
					s1_odr: odr1,
					s1_temperature: signInt(d.slice(2, 3).reduce(msbLsb), 16) / 100,

					x1_rms: signInt(d.slice(4, 5).reduce(msbLsb), 16),
					x1_max: signInt(d.slice(6, 7).reduce(msbLsb), 16),
					x1_velocity: signInt(d.slice(8, 9).reduce(msbLsb), 16) / 100,
					x1_displacement: signInt(d.slice(10, 11).reduce(msbLsb), 16) / 100,
					x1_peak_one: signInt(d.slice(12, 13).reduce(msbLsb), 16),
					x1_peak_two: signInt(d.slice(14, 15).reduce(msbLsb), 16),
					x1_peak_three: signInt(d.slice(16, 17).reduce(msbLsb), 16),

					y1_rms: signInt(d.slice(18, 19).reduce(msbLsb), 16),
					y1_max: signInt(d.slice(20, 21).reduce(msbLsb), 16),
					y1_velocity: signInt(d.slice(22, 23).reduce(msbLsb), 16) / 100,
					y1_displacement: signInt(d.slice(24, 25).reduce(msbLsb), 16) / 100,
					y1_peak_one: signInt(d.slice(26, 27).reduce(msbLsb), 16),
					y1_peak_two: signInt(d.slice(28, 29).reduce(msbLsb), 16),
					y1_peak_three: signInt(d.slice(30, 31).reduce(msbLsb), 16),

					z1_rms: signInt(d.slice(32, 33).reduce(msbLsb), 16),
					z1_max: signInt(d.slice(34, 35).reduce(msbLsb), 16),
					z1_velocity: signInt(d.slice(36, 37).reduce(msbLsb), 16) / 100,
					z1_displacement: signInt(d.slice(38, 39).reduce(msbLsb), 16) / 100,
					z1_peak_one: signInt(d.slice(40, 41).reduce(msbLsb), 16),
					z1_peak_two: signInt(d.slice(42, 43).reduce(msbLsb), 16),
					z1_peak_three: signInt(d.slice(44, 45).reduce(msbLsb), 16),

					s2_odr: odr2,
					s2_temperature: signInt(d.slice(47, 48).reduce(msbLsb), 16) / 100,

					x2_rms: signInt(d.slice(49, 50).reduce(msbLsb), 16),
					x2_max: signInt(d.slice(51, 52).reduce(msbLsb), 16),
					x2_velocity: signInt(d.slice(53, 54).reduce(msbLsb), 16) / 100,
					x2_displacement: signInt(d.slice(55, 56).reduce(msbLsb), 16) / 100,
					x2_peak_one: signInt(d.slice(57, 58).reduce(msbLsb), 16),
					x2_peak_two: signInt(d.slice(59, 60).reduce(msbLsb), 16),
					x2_peak_three: signInt(d.slice(61, 62).reduce(msbLsb), 16),

					y2_rms: signInt(d.slice(63, 64).reduce(msbLsb), 16),
					y2_max: signInt(d.slice(65, 66).reduce(msbLsb), 16),
					y2_velocity: signInt(d.slice(67, 68).reduce(msbLsb), 16) / 100,
					y2_displacement: signInt(d.slice(69, 70).reduce(msbLsb), 16) / 100,
					y2_peak_one: signInt(d.slice(71, 72).reduce(msbLsb), 16),
					y2_peak_two: signInt(d.slice(73, 74).reduce(msbLsb), 16),
					y2_peak_three: signInt(d.slice(75, 76).reduce(msbLsb), 16),

					z2_rms: signInt(d.slice(77, 78).reduce(msbLsb), 16),
					z2_max: signInt(d.slice(79, 80).reduce(msbLsb), 16),
					z2_velocity: signInt(d.slice(81, 82).reduce(msbLsb), 16) / 100,
					z2_displacement: signInt(d.slice(83, 84).reduce(msbLsb), 16) / 100,
					z2_peak_one: signInt(d.slice(85, 86).reduce(msbLsb), 16),
					z2_peak_two: signInt(d.slice(87, 88).reduce(msbLsb), 16),
					z2_peak_three: signInt(d.slice(89, 90).reduce(msbLsb), 16),
				};
			}
		},
		'101':{
