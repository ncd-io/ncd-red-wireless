...
module.exports = class WirelessSensor{
	...
	sensor_data(payload, frame){
		...
		if(parsed.sensor_type == 101){
			var packet_id = payload[4];
			var expected_packets = payload[14];
			console.log('------------');
...
// console.log('battery [2], [3], msblsb');
// console.log(payload[2]);
// console.log(payload[3]);
// console.log(msbLsb(payload[2], payload[3]));
// console.log('packet count');
// console.log(payload[4]);
// console.log('device type');
// console.log(msbLsb(payload[5], payload[6]));
// console.log('reserve byte');
// console.log(payload[7]);
// console.log('mode of operation');
// console.log(payload[8]);
console.log('odr');
console.log(payload[9]);
console.log('enabled axis');
...
// console.log(msbLsb(payload[13], payload[14]));
console.log('Total Number of RF Packets');
console.log(payload[15]);
// console.log('Current Packet Count value');
// console.log(payload[16]);
// console.log('X-Axis RAW ACC Data');
// console.log(payload[17]);
// console.log(payload[18]);
// console.log(msbLsb(payload[17], payload[18]));
// console.log('Y-Axis RAW ACC Data');
// console.log(payload[19]);
// console.log(payload[20]);
// console.log(msbLsb(payload[19], payload[20]));
// console.log('Y-Axis RAW ACC Data');
// console.log(payload[21]);
// console.log(payload[22]);
// console.log(msbLsb(payload[21], payload[22]));
			...
			if(payload[8] == 70 && payload[9] == 76 && payload[10] == 89){
				...
				// setTimeout(function(){this.config_set_rtc_101('00:00:00:00:00:00:FF:FF')}.bind(this), 1000);

				// untried
				// setTimeout(function(that){this.config_set_rtc_101}, 1000, '00:00:00:00:00:00:FF:FF');
			...
			if(deviceAddr in globalDevices){
				...
				if(payload[16] in globalDevices[deviceAddr].data || payload[16] == 1 || !(((payload[16]&127)-1) in globalDevices[deviceAddr].data)) {
					console.log('bad packet breakdown deleting');
					console.log(payload[16] in globalDevices[deviceAddr].data);
					console.log(payload[16] == 1);
					console.log(!((payload[16]-1) in globalDevices[deviceAddr].data));
					// console.log(globalDevices[deviceAddr].data);
					delete globalDevices[deviceAddr];
					return;
				}
				else{
					globalDevices[deviceAddr].data[payload[16]] = payload.slice(17);
				}
				...
				if(Object.keys(globalDevices[deviceAddr].data).length == (globalDevices[deviceAddr].stream_size)){
					...
					for(var i = 0; i < raw_data.length; i+=en_axis_data.increment){
						...
						var fft_data = {
							...
							reading: label,
							// x: signInt(((raw_data[i]<<8)+(raw_data[i+1]&255)), 16)*.00006,
							// y: signInt(((raw_data[i+2]<<8)+(raw_data[i+3]&255)), 16)*.00006,
							// z: signInt(((raw_data[i+4]<<8)+(raw_data[i+5]&255)), 16)*.00006,
							odr: globalDevices[deviceAddr].odr,
							temperature: globalDevices[deviceAddr].temperature,
							// operation_mode: globalDevices[deviceAddr].mo,
							en_axis: globalDevices[deviceAddr].en_axis,
							mac_address: deviceAddr,
							// accelerometer_x_1_raw: (raw_data[i]),
							// accelerometer_x_1: (raw_data[i]<<8),
							// accelerometer_x_2: (raw_data[i+1]),
							// accelerometer_y_1_raw: (raw_data[i+2]),
							// accelerometer_y_1: (raw_data[i+2]<<8),
							// accelerometer_y_2: (raw_data[i+3]),
							// accelerometer_z_1_raw: (raw_data[i+4]),
							// accelerometer_z_1: (raw_data[i+4]<<8),
							// accelerometer_z_2: (raw_data[i+5]),
						}
						console.log('en_axis_data');
						console.log(en_axis_data);

						if('x_offset' in en_axis_data){
							console.log('x_offset');
							fft_data.x = signInt(((raw_data[i+en_axis_data.x_offset]<<8)+(raw_data[i+en_axis_data.x_offset+1])), 16)*.00006;
						}
						if('y_offset' in en_axis_data){
							console.log('y_offset');

							fft_data.y = signInt(((raw_data[i+en_axis_data.y_offset]<<8)+(raw_data[i+en_axis_data.y_offset+1])), 16)*.00006;
						}
						if('z_offset' in en_axis_data){
							console.log('z_offset');
							fft_data.z = signInt(((raw_data[i+en_axis_data.z_offset]<<8)+(raw_data[i+en_axis_data.z_offset+1])), 16)*.00006;
						}
					...
					console.log(JSON.stringify(fft.test));
					parsed.concat_data = fft_concat_obj;
				...
				globalDevices[deviceAddr] = {
					...
					hour: payload[11],
					minute: payload[12],
					temperature: msbLsb(payload[13], payload[14]),
				}

				globalDevices[deviceAddr].data[payload[16]] = payload.slice(17);
				// console.log('packet info:')
				// console.log(payload[16]);
				// console.log(payload.slice(17).length);
				// console.log(payload.slice(17).join(', '));
