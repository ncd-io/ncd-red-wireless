...
function sensor_types(parent){
	var types = {
		...
		'31': {
			'name': 'TVOC CO2eq Temperature Humidity Gas',
			parse: (d) => {
				return {
					humidity:		d.slice(0,2).reduce(msbLsb),
					temperature:	signInt(d.slice(2,4).reduce(msbLsb), 16),
					co2eq:			signInt(d.slice(4,6).reduce(msbLsb), 16),
					tvoc:			signInt(d.slice(6,8).reduce(msbLsb), 16),
					h2:				signInt(d.slice(8,10).reduce(msbLsb), 16),
					ethanol:		signInt(d.slice(10,12).reduce(msbLsb), 16)
				}
			}
		},
		'32': {
		...
		'34': {
			...
			parse: (d) => {
				console.log(d);
				return {
					depth_mm: d.slice(0, 4).reduce(msbLsb)
				};
			}
		},
		'35': {
